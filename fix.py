...
def rate_monotonic_scheduling(tasks, hyper_period=20):
    ...
    return gantt_chart, waiting_times
def earliest_deadline_first(tasks, hyper_period=20):
    """
    Earliest Deadline First Scheduling.
    Args:
        tasks (list): A list of tasks, where each task is a tuple 
                      (task_name, burst_time, period, deadline, arrival_time).
        hyper_period (int): The simulation period.
    Returns:
        tuple: (gantt_chart, waiting_times, average_waiting_time, missed_deadlines)
    """
    # Validation des tâches
    for i, task in enumerate(tasks):
        if len(task) < 5:
            raise ValueError(f"La tâche {i} est incomplète : {task}")

    # Tri initial des tâches par temps d'arrivée
    tasks.sort(key=lambda x: x[4])

    current_time = 0
    gantt_chart = []
    remaining_burst = {task[0]: task[1] for task in tasks}  # Temps de burst restant
    deadlines = {task[0]: task[3] for task in tasks}  # Deadlines initiales
    waiting_times = {task[0]: 0 for task in tasks}  # Temps d'attente initial
    missed_deadlines = []  # Liste des deadlines manquées

    while current_time < hyper_period:
        # Filtrer les tâches prêtes à exécuter
        ready_tasks = [
            task for task in tasks if remaining_burst[task[0]] > 0 and task[4] <= current_time
        ]

        # Trier les tâches prêtes par deadline la plus proche
        ready_tasks.sort(key=lambda x: deadlines[x[0]])

        if ready_tasks:
            # Exécuter la tâche avec la deadline la plus proche
            next_task = ready_tasks[0]
            task_name, burst_time, period, deadline, arrival_time = next_task

            gantt_chart.append((task_name, current_time, current_time + 1))
            remaining_burst[task_name] -= 1

            # Vérifier si la tâche a dépassé sa deadline
            if current_time >= deadlines[task_name]:
                missed_deadlines.append(task_name)

            # Réinitialiser la tâche si elle est terminée
            if remaining_burst[task_name] == 0:
                remaining_burst[task_name] = burst_time
                deadlines[task_name] += period

            # Mettre à jour le temps d'attente pour les autres tâches prêtes
            for task in ready_tasks:
                if task[0] != task_name:
                    waiting_times[task[0]] += 1
        else:
            # Idle si aucune tâche n'est prête
            gantt_chart.append(("Idle", current_time, current_time + 1))
    ...
    average_waiting_time = total_waiting_time / len(waiting_times) if waiting_times else 0

    return gantt_chart, waiting_times, average_waiting_time, missed_deadlines
...
algorithm = st.selectbox(
    "Select Scheduling Algorithm",
    ["FCFS (First Come First Serve)", "SJF Preemptive", "SJF Non-Preemptive", "Rate Monotonic","Earliest Deadline First (EDF)"]
)
...
if st.button("Simulate"):
    ...
    if algorithm == "FCFS (First Come First Serve)":
        ...
        gantt_chart, waiting_times, avg_waiting_time, missed_deadlines = earliest_deadline_first(tasks, hyper_period=hyperperiod)
